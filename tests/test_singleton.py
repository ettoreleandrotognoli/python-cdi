...
class SingletonTest(TestCase):
    ...
    def test_random(self):
        singletons = []
        for _ in range(SINGLETON_LIMIT):
    ...
    def test_legacy_function_with_type(self):
        @Singleton(produce_type=float)
        def get_singleton():
            return random()

        singletons = [
            DEFAULT_CONTAINER.produce(float)
            for x in range(SINGLETON_LIMIT)
        ]
    ...
    def test_function_without_type(self):
        ...
        singletons = [
            DEFAULT_CONTAINER.produce(object)
            for x in range(SINGLETON_LIMIT)
        ]
        self.assertEqual(len(set(singletons)), 1)
